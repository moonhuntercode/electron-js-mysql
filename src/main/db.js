import mysql from 'mysql2/promise' // Usamos mysql2 para crear la conexión temporal
...
async function initializeDatabase() {
  ...
  try {
    ...
    // Cerrar la conexión temporal, incluso si ocurre un error
    if (connection) {
      try {
        await connection.end()
        console.log('✅ Conexión temporal cerrada.')
      } catch (closeError) {
        console.error('❌ Error al cerrar la conexión temporal:', closeError)
      }
    }
  }
}
...
/**
 * Función para inicializar Sequelize y probar la conexión.
 */
export async function initializeSequelize() {
  try {
    ...
    if (error.name === 'SequelizeConnectionError') {
      console.error(
        '❌ Error de conexión a la base de datos. Verifica que MySQL esté en ejecución y las credenciales en .env.'
      )
